...
@pytest.fixture
def movement_intensity_dataframe_with_mean_1() -> Generator[pd.DataFrame, Any, Any]:
    # 1000 elements to make the mean close enough to 1 when a new (regular) value is added
    acceleration_values_in_x_axis = [1.0] * 1000
    df = pd.DataFrame({
...
def test_mean_total_acceleration_when_object_is_absolutely_still():
    ...
    MovementAnalyser.create_total_acceleration_column(df)
...
def test_classifying_low_intensity_movement(movement_intensity_dataframe_with_mean_1: pd.DataFrame):
    ...
    MovementAnalyser.create_total_acceleration_column(movement_intensity_dataframe_with_mean_1)
